        ...
        <div class="page-container">
                <div class="data-list">
                                ...
                                <el-form :inline="true" :model="filters" :size="size">
                                        <el-form-item>
                                                <kt-button icon="fa fa-refresh" :label="$t('action.refresh')"
                                                        perms="StudentAllshow:view" type="primary" @click="handleQuery" />
                                        </el-form-item>
                                        <el-form-item>
                                                <kt-button icon="fa fa-plus" :label="$t('action.add')"
                                                        perms="StudentAllshow:add" type="primary" @click="handleAdd" />
                                        </el-form-item>
                        ...
                        <el-table :data="tableData" stripe size="mini" style="width: 98%" border
                                ...
                                <el-table-column prop="fmajor" align="center" label="排序数值"></el-table-column>
                                ...
                                <el-table-column fixed="right" align="center" width="185"
                                        ...
                                        <template slot-scope="scope">
                                                <kt-button icon="fa fa-edit" :label="$t('action.edit')"
                                                        perms="StudentAllshow:edit" @click="handleEdit(scope.row)" />
                                                <kt-button icon="fa fa-trash" :label="$t('action.delete')"
                                                        perms="StudentAllshow:delete" type="danger"
                ...
                <el-dialog :title="!dataForm.id ? '显示所有信息--新增' : '显示所有信息--修改'" width="50%" :visible.sync="dialogVisible"
                        ...
                        <el-form :model="dataForm" :rules="dataRules" ref="dataForm" @keyup.enter.native="submitForm()"
                                ...
                                <el-form-item label="排序数值" prop="fmajor">
                                        <el-input v-model="dataForm.fmajor" placeholder="排序数值"></el-input>
                                </el-form-item>
...
export default {
        ...
        data() {
                return {
                        ...
                        dataRules: {
                                ...
                                fmajor: [{ required: true, message: "排序数值不能为空！", trigger: "blur" },
                                ],
        ...
        methods: {
                find: function () {
                        ...
                        this.$api.StudentAllshow.find(queryConditions)
                                .then((res) => {
                                        if (res.code == 200) {
                ...
                handleDelete(row) {
                        ...
                        this.$confirm(
                                ...
                                this.$api.StudentAllshow.deleteById(params.id)
                                        .then((res) => {
                                                if (res.code == 200) {
                                                        this.find();
                ...
                submit() {
                        this.$refs["dataForm"].validate((valid) => {
                                if (valid) {
                                        ...
                                        this.$api.StudentAllshow.save(params).then((res) => {
                                                if (res.code == 200) {
