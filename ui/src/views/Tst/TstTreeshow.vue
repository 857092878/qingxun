...
<el-table-column prop="fcode" align="center" label="部门编码"></el-table-column>
<el-table-column prop="fname" align="center" label="部门名称"></el-table-column>
...
icon="fa fa-edit"
:label="$t('action.edit')"
perms="TstTreeshow:edit"
@click="handleEdit(scope.row)"
/>
<kt-button
icon="fa fa-trash"
:label="$t('action.delete')"
perms="TstTreeshow:delete"
type="danger"
...
<!-- 新增修改界面 -->
<el-dialog
:title="!dataForm.id ? '树形显示信息--新增' : '树形显示信息--修改'"
width="50%" :visible.sync="dialogVisible" :close-on-click-modal="false">
<el-form :model="dataForm" :rules="dataRules" ref="dataForm"
@keyup.enter.native="submitForm()" center label-width="80px"
:size="size" style="text-align: left">
<el-form-item label="部门编码" prop="fcode">
<el-input v-model="dataForm.fcode" placeholder="部门编码"></el-input>
</el-form-item>
<el-form-item label="部门名称" prop="fname">
<el-input v-model="dataForm.fname" placeholder="部门名称"></el-input>
</el-form-item>
...
fcode: [{ required: true, message: "部门编码不能为空！", trigger: "blur" },
{ max:20, message: "部门编码最大长度为20个字符长度！", trigger: "blur" },
],
fname: [{ required: true, message: "部门名称不能为空！", trigger: "blur" },
{ max:20, message: "部门名称最大长度为20个字符长度！", trigger: "blur" },
],
...
this.$api.TstTreeshow.deleteById(params.id)
.then((res) => {
if (res.code == 200) {
this.findTree();
...
this.$api.TstTreeshow.save(params).then((res) => {
if (res.code == 200) {
