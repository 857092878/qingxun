...
@Service
public class StudentAllshowServiceImpl extends CurdServiceImpl<StudentAllshow> {

    @Override
    protected void insertUpdateCheck(StudentAllshow object, Boolean update) {
    ...
    public List<PairedData> findChoice() {
        String sql = "select id,fname from student_allshow";// order by sortNumber";
        List<StudentAllshow> ls = selectBySQL(sql);
        Integer len = ls.size();
