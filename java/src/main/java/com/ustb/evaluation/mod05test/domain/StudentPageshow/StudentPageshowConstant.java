package com.ustb.evaluation.mod05test.domain.StudentPageshow;
...
@Component
public class StudentPageshowConstant extends TableConstant<StudentPageshow> {
    public StudentPageshowConstant() {
        super();
    }
    ...
    public void initFirtSevenData() {
        ...
        selectFrom = " from student_pageshow a left outer join student_treeshow b on a.deptId = b.id left outer join student_allshow c on a.fnation = c.id";
        selectFieldsSimple = selectFields;
        selectFromSimple = selectFrom;
        selectFieldsComplex = selectFields;
        selectFromComplex = selectFrom;
        tableName = "student_pageshow";//表单名
        hasSafe = false;
    }
