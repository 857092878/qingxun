...
import com.ustb.evaluation.mod05test.service.impl.TstAllshowServiceImpl;
...
@Api(tags = {"全部显示"})
@RestController
@RequestMapping("/v1/tst/TstAllshow")
@CrossOrigin
public class TstAllshowController extends BaseController<TstAllshow> {

    @Autowired
    private TstAllshowServiceImpl tstAllshowService;
    ...
    @PostMapping("/insert")
    public ApiResult insert(@Valid @RequestBody TstAllshow object, BindingResult result) {
        if (result.hasErrors()) {
    ...
    @PostMapping("/update")
    public ApiResult update(@Valid @RequestBody TstAllshow object, BindingResult result) {
        if (result.hasErrors()) {
            List<FieldError> ls = result.getFieldErrors();
            throw new PromptException(result.getFieldErrors().get(0).getDefaultMessage());
        }
    ...
    protected ApiResult find(@RequestBody QueryConditions queryConditions) {
        ...
        List<TstAllshow> ls = (List<TstAllshow>) ar.getData();
        if (ls != null) {
    ...
    public ApiResult findChoice() {
        List<PairedData> lspd = tstAllshowService.findChoice();
